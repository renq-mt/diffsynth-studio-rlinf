"""
Stream prediction evaluation: autoregressive generation using model's own predicted history.
The model never sees GT frames after the first frame — it rolls forward using its own outputs.

Usage:
    python examples/wanvideo/model_eval/stream_prediction.py \
        --model_ckpt <path> \
        --vae_ckpt <path> \
        --data_path <episode_dir> \
        --output_path outputs/eval/stream \
        --total_frames 505 \
...
def parse_args():
    ...
    p.add_argument("--output_path", default="outputs/eval/stream")
...
def load_episode(data_path, total_frames, device):
    ...
    # Support both [T, cam, C, H, W] and [T, C, H, W]
    if rgb.ndim == 5:
        rgb = rgb[:, 0]  # take first camera
    # rgb: [T, C, H, W] or [T, H, W, C]
    if rgb.shape[1] == 3:  # [T, C, H, W] → [T, H, W, C]
    ...
    if actions.ndim == 3:
        actions = actions[:, 0]  # [T, cam, 7] → [T, 7]
    action_full = torch.from_numpy(actions[:total_frames]).float()
    # Zero out first frame action, mark gripper as -1 (convention from training)
    action_full[0] = 0
    action_full[0, -1] = -1
...
def run_stream_prediction(pipe, gt_frames, action_full, args):
    """
    Autoregressive prediction: after the first frame, the model only sees its own outputs.
    """
    total_frames = args.total_frames
    num_iters = (total_frames - 1) // (WINDOW - 1)

    generated_frames = []
    # input_image: first GT frame (only GT frame ever used)
    input_image = gt_frames[0]
    # input_image4: last 4 context frames, initialized from GT first frame
    ...
    for i in range(num_iters):
        ...
        # Use model's own output as next context — no GT leakage
...
def main():
    ...
    print(f"Running stream prediction ({args.total_frames} frames)...")
    pred_frames = run_stream_prediction(pipe, gt_frames, action_full, args)

    # Save predicted video
    pred_path = os.path.join(args.output_path, "pred_stream.mp4")
    ...
    # Save frames as npy for metric computation
    def frames_to_npy(frames):
        return np.stack([np.array(f) for f in frames])  # [T, H, W, 3]

    np.save(os.path.join(args.output_path, "pred_stream.npy"), frames_to_npy(pred_frames))
